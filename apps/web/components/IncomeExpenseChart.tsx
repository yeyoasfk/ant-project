...
export default function IncomeExpenseChart({ transactions }: { transactions: any[] }) {
 ...
 // 🧠 Lógica para filtrar y agrupar transacciones según la pestaña seleccionada
  ...
  const chartOptions: any = {
    ...
    scales: {
      y: { border: { display: false }, grid: { color: '#e5e7eb', borderDash: [5, 5] }, ticks: { color: '#9ca3af', callback: (value: any) => `$${value/1000}k` } },
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex justify-center md:justify-start">
        <div className="flex bg-gray-100 p-1 rounded-full border border-gray-200 shadow-sm w-full md:w-auto overflow-x-auto">
          {['dia', 'semana', 'mes', 'año'].map((tab) => (
            <button
              key={tab}
              onClick={() => setTimeframe(tab as any)}
              className={`flex-1 md:flex-none px-6 py-2 rounded-full text-14 font-semibold capitalize transition-all ${
                timeframe === tab ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:text-gray-900'
              }`}
            >
              {tab}
            </button>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between mb-6 relative">
          <h2 className="text-18 font-bold text-gray-900">Ingresos & Egresos</h2>
          
          <div className="flex gap-2">
            <button className="flex items-center justify-center size-9 rounded-full bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors">
              <Search className="size-4" />
            </button>
            <button 
              onClick={() => setShowCalendar(!showCalendar)}
              className="flex items-center justify-center size-9 rounded-full bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors"
            >
            ...
            {showCalendar && (
              <div className="absolute right-0 top-12 z-10 w-64 rounded-xl border border-gray-200 bg-white p-4 shadow-lg">
                <h3 className="text-12 font-bold text-gray-700 mb-2">Rango de Días (Max 7)</h3>
                <div className="flex flex-col gap-2">
                  <input type="date" className="text-sm border rounded-md p-1.5" onChange={e => setCustomRange({...customRange, start: e.target.value})} />
                  <input type="date" className="text-sm border rounded-md p-1.5" onChange={e => setCustomRange({...customRange, end: e.target.value})} />
                  <button className="bg-blue-600 text-white rounded-md py-1.5 text-sm font-semibold mt-1 hover:bg-blue-700" onClick={() => setShowCalendar(false)}>Aplicar</button>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="flex flex-row items-center justify-between md:justify-around rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        {/* Total Ingreso */}
        <div className="flex flex-col items-center gap-2 w-1/2 border-r border-gray-100">
          <div className="flex items-center gap-1.5">
            <div className="bg-green-100 p-1.5 rounded-md">
              <ArrowUpRight className="size-5 text-green-600" />
            </div>
            <p className="text-14 font-semibold text-gray-600">Ingreso</p>
          </div>
          <p className="text-24 font-bold text-gray-900">{formatAmount(totals.ingreso)}</p>
        </div>
        ...
        <div className="flex flex-col items-center gap-2 w-1/2">
          <div className="flex items-center gap-1.5">
            <div className="bg-blue-100 p-1.5 rounded-md">
              <ArrowDownRight className="size-5 text-blue-600" />
            </div>
            <p className="text-14 font-semibold text-gray-600">Egreso</p>
          </div>
          <p className="text-24 font-bold text-blue-600">{formatAmount(totals.egreso)}</p>
        </div>
      </div>
    </div>
  );

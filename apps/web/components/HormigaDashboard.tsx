...
export default function HormigaDashboard({
  ...
  // ── Slicing for table display ─────────────────────────────────────────────
  const displayedData = isExpanded ? filteredTableData : filteredTableData.slice(0, INITIAL_LIMIT);
  const hasMore = filteredTableData.length > INITIAL_LIMIT;
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-5xl">
      ...
      <div className="flex flex-col gap-3">
        ...
        {/* Table */}
        <GlassContainer size="md" className="!p-0 overflow-hidden relative">
          {/* Column headers */}
          ...
          {hasMore && (
            <div className="flex justify-center p-4 border-t border-white/5 bg-gradient-to-t from-[#1a0b21]/80 to-transparent">
              <button
                ...
                className="flex items-center gap-2 px-6 py-2 rounded-full border border-white/10 bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white transition-all text-sm font-semibold shadow-sm hover:shadow-[0_0_15px_rgba(101,53,132,0.4)]"
              >
                {isExpanded 
                  ? 'Mostrar menos 🔼' 
                  : `Mostrar más (${filteredTableData.length - INITIAL_LIMIT}) 🔽`
                }
              </button>
            </div>
          )}
